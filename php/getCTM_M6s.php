...
$log->i("partition={$partition} user={$user} ctmbitwise={$ctmbitwise} ctmreport={$ctmreport} $fromdate $todate $searchnumber");
...
if ( $fromdate ===  NULL ){
	$sql = "select TimeStart from M6s order by TimeStart asc limit 1";
	$rows = $db -> select($sql);
	$fromdatedisplay = $rows[0]['TimeStart'];
}

if ( $todate ===  NULL ){
	$sql = "select TimeStart from M6s order by TimeStart desc limit 1";
	$rows = $db -> select($sql);
...
if ( $ctmreport == 1 )
{
	...
	from M6s
	where RecordType = 0
	and TerminationReason = 0
	and CallDirection = 'O'
	and callType in ('INTERSTATE','INTL','INTRASTATE','TOLLFREE','WZ1')";
...
if ( $fromdate != "" ) { $fromdatedisplay = $fromdate; }
if ( $todate != "" ) { $todatedisplay = $todate; }

echo json_encode(array("data" => $result, "data2" => $result2, "data3" => $result3, "data4" => $result4, "count" => $count, "fromdate" => $fromdatedisplay, "todate" => $todatedisplay));